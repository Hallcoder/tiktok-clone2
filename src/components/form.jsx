...
function Form({onLoad}) {
       ...
       switch(input.name){
         case 'caption':
          if(input.value.length <= 0) {
              errors1[input.name] = "At least a character in the field must be specified";
  ...
  return (
    <form method="post" onSubmit={upload} className="flex h-full m-auto  w-11/12  mt-24">
      <div id="videoForm" className="border-2 items-center shadow-md w-4/12 ml-auto mr-auto mt-10 text-xl h-5/6 flex flex-col rounded-md min-w-1/2">
            ...
            <div className="border-2 hover:border-red-600 border-dashed w-8/12 rounded-md ml-8 h-full flex flex-col justify-between items-center">
              ...
              <input type="file" onChange={videoChange} className="w-9/12 mb-6 font-semibold cursor-pointer file:cursor-pointer text-sm text-red-600 h-8 file:rounded-full file:h-5/6 file:bg-red-400 file:border-0 file:text-white file:font-bold" />
            </div>
     </div>
      <div id="wordForm">
      <div className="flex flex-col h-16  w-11/12  ml-4 mt-4">
        ...
        />
        {errors['caption'] ? <ErrorBadge message={errors['caption']} />:<SuccessBadge />}
      </div>
      ...
      <p className="text-xs w-9/12 ml-4 inline-block text-gray-300">
        ...
        posting.<NavLink to="#" className="text-sm text-black hover:underline">Learn more</NavLink>
      </p>
