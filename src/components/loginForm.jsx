...
function Login({Display}){
  ...
  let [errors, setErrors] = useState({ email: "", password: "" ,submissionError:""});
  ...
  if(page === 'emailPage'){
        ...
        <div>
          ...
          >
            ...
            <div>
              ...
              {errors.password.message && (<div className="h-8 m-auto w-10/12 rounded-md text-red-500 bg-red-200">{errors.password.message}</div>)}
            </div>
            {errors.submissionError && <div><ErrorBadge message={errors.submissionError} /></div>}
            ...
            <input
              ...
              type="submit"
              value="Log in"
              className={(errors.email.message || errors.password.message) ? "w-10/12 h-12 font-bold border-2 rounded-sm m-auto":"w-10/12 h-12 font-bold  cursor-pointer text-white bg-red-500 rounded-sm m-auto"}
            />
          </form>
        </div>
    ...
    return(
      <ImageUpload  image={image} setImage={setImage} Display={Display} changePage={changePage}/>
    )
  }
}
